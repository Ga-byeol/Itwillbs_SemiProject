...
<body>
	...
	<table class="table_contents" border="1">
		<tr>
			<td colspan="7"><%=dto.getNotice_sub()%></td>
		</tr>
		<tr>
			<td>작성자</td>
		...
		<tr>
			...
			<td colspan="7" style="text-align: center;">
				...
				if (viewFile.exists()) {
				%> <img alt="img"
				src="notice_images/<%=dto.getNotice_image()%>" width="300">
			</td>
		</tr>
		<%
		}
		...
		<tr>
			<td>글내용</td>
			<td colspan="7"><%=dto.getNotice_content()%></td>
		</tr>
	...
	<input type="button" value="글수정"
		onclick="location.href='./NoticeUpdateForm.no?num=<%=dto.getNotice_num()%>'">

	<form action="./NoticeDelete.no" method="get" class="Dconfirm">
		<input type="hidden" name="num" value=<%=dto.getNotice_num()%>>
		<input type="hidden" name="notice_image"
			value=<%=dto.getNotice_image()%>> <input type="submit"
			value="글삭제">
	</form>
	...
	<table border="1">
		<tr onclick="location.href='./NoticeContent.no?num=<%=dto2.getNotice_num()%>'"
	...
	<table border="1">
		<tr onclick="location.href='./NoticeContent.no?num=<%=dto3.getNotice_num()%>'"
			style="cursor: Pointer">
			<td>이전공지사항</td>
